...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
    ...
    public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) throws BeansException {
        ...
        // 对bean进行增强，创建代理，返回代理对象，压制后续bean的默认实例化行为
        if (beanName != null) {
    ...
    protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
        ...
        // Create proxy if we have advice.   获取通知数组
    ...
    protected Object createProxy(Class<?> beanClass, String beanName, Object[] specificInterceptors, TargetSource targetSource) {
        ...
        // 是否代理目标类，也就是对<aop:config>节点的proxy-target-class属性进行判断;proxyFactory.isProxyTargetClass()默认为false
        if (!proxyFactory.isProxyTargetClass()) {
            //通过CGLIB进行代理
            if (shouldProxyTargetClass(beanClass, beanName)) {
                logger.debug("will createProxy for bean: " + beanName + " with CgLib");
                proxyFactory.setProxyTargetClass(true);
            } else {
                //通过JDK进行代理，需要实现接口
                logger.debug("will createProxy for bean: " + beanName + " with JdkInterfaceProxy");
        ...
        logger.debug("then buildAdvisors.. for bean: " + beanName + " advisors:" + advisors);
        ...
        logger.debug("end getProxy.. for instead bean: " + beanName);
